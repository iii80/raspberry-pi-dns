...
大家好，我是卡卡。由于家里的网络总是被运营商进行 DNS 污染，本来能访问的域名总是被屏蔽掉，于是萌生了利用树莓派做个 DNS 解析的想法，现在实现了树莓派上解析 DNS，自动区分网站是否被墙，实现自动代理。配置过程中也走了很多弯路，在网上教程没有找到一个比较系统的配置方法，于是把自己的经验与大家分享一下，希望能对大家有所帮助（能点个 star 支持就更好了）。本教程的实验环境为树莓派，理论上只要是 Linux 系统就可以，可能细节步骤有所差别。教程仅限于交流学习，禁止用于非法用途。



另外由于作者水平原因，教程难免有疏漏错误之处，如果有问题大家可以在 issue 交流或者加 qq 群：`1015256667`（进群答案为 github)
...
经过上面的步骤，我们应该可以上 Google 了，dns 也不会被污染。这里说一些小技巧，我们可以在上面的 ipset 中设置一个广告集合，将广告的网址我们转发到一个不能解析的端口，这样就可以实现屏蔽广告。